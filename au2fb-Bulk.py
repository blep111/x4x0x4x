...
def banner():
    os.system("clear")
    print(f"{W}<{R}•{W}> BULK FACEBOOK CREATE TOOLS PAID🧑‍💻")
    print(f"{W}<{R}•{W}> AUTHOR :- {G}XXXX 404")
    print(f"{W}———————————————————————————————")
...
def main() -> None:
    ...
    for make in range(100):
        ...
        payload = {
            ...
            'guid': "",
            'pre_form_step': "",
            'encpass': '#PWD_BROWSER:0:{}:{}'.format(str(time.time()).split('.')[0],"xx404##321"),
            'submit': "Sign Up",
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            print(f"{X}{R} TH ID DISABLED")
            linex()
        else:
            cookie = (";").join([ "%s=%s" % (key,value) for key,value in ses.cookies.get_dict().items()])
            print(f"{X} SUCCESS - {G}{uid}|xx404##321|{cookie}")
            open("/sdcard/SUCCESS-OK-ID.txt","a").write(uid+"|xx404##321|"+cookie+"\n")
            linex()
    except Exception as e:
        linex()
        pass
